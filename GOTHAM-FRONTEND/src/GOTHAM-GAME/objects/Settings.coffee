...
class Settings extends Gotham.Graphics.Container
  ...
  create: ->
    ...
    topBar_Close.y = 0
    topBar_Close.setInteractive true
    ...
    topBar.mousedown = topBar.touchstart = (data) ->
      @data = data
      @sx = data.getLocalPosition(@parent).x * spriteContainer.scale.x;
      @sy = data.getLocalPosition(@parent).y * spriteContainer.scale.y;
      @dragging = true


    topBar.mouseup = topBar.mouseupoutside = topBar.touchend = topBar.touchendoutside = (data) ->
      @data = null
      @dragging = false

    topBar.mousemove = topBar.touchmove = (data) ->
      if @dragging
        newPosition = @data.getLocalPosition(spriteContainer.parent);
    ...
    soundLevel.onProgress = (progress) ->
      ...
      for key in Object.keys(Gotham.Preload.storage.audio)
        Gotham.Preload.storage.audio[key].volume (progress/100)
